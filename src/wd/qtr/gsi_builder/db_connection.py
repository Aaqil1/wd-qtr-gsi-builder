...
class RedshiftConnection:
    ...
    @staticmethod
    def _filing_impact_condition(alias="ec"):
        return f"COALESCE(LOWER(CAST({alias}.impacts_filing AS VARCHAR)), '') IN ('true', 't', '1', 'y', 'yes')"

    def _query_table_columns(self, table_name):
        ...
        """
        try:
            df = self._execute_query(query, timeout_seconds=60)
            return {row["column_name"] for row in df.collect()}
        except Exception as e:
            logger.warning(f"Could not inspect columns for {self.schema}.{table_name}: {e}")
            return set()

    def query_company_filing_error_count(self, site_id, branch_code, company_code, year, quarter) -> int:
        """Count company errors whose catalog row has filing impact enabled."""
        site_id = self._validate_input(site_id)
        ...
        query = f"""
        SELECT COUNT(DISTINCT ce.company_error_sk) AS error_count
        FROM {self.schema}.company_errors ce
        INNER JOIN {self.schema}.error_catalog ec
            ON ce.error_code = ec.error_code
        INNER JOIN {self.schema}.organization_unit ou
            ON ce.organization_unit_sk = ou.organization_unit_sk
        WHERE ou.site_id = '{site_id}'
          AND ou.branch_code = '{branch_code}'
          AND ou.company_code = '{company_code}'
          AND UPPER(COALESCE(ce.resolution_status, '')) = 'OPEN'
          AND UPPER(COALESCE(ce.write_status, '')) = 'PENDING'
          AND {self._filing_impact_condition("ec")}
          AND EXISTS (
              SELECT 1
              FROM {self.schema}.worker_tax_qtr_snapshot wt
              WHERE wt.organization_unit_sk = ce.organization_unit_sk
                AND (ce.payroll_run_sk IS NULL OR wt.payroll_run_sk = ce.payroll_run_sk)
                AND wt.year = '{year}'
                AND wt.quarter = '{quarter}'
          )
        """
        df = self._execute_query(query, timeout_seconds=120)
        return int(df.collect()[0]["error_count"])

    def query_worker_filing_error_worker_sks(self, site_id, branch_code, company_code, year, quarter):
        """Find worker IDs to skip from worker-level error tables when present.

        Candidate worker error tables are optional because their exact production
        names are not defined in this repo. Tables that do not exist, or do not
        contain the required worker/error columns, are ignored.
        """
        site_id = self._validate_input(site_id)
        branch_code = self._validate_input(branch_code)
        company_code = self._validate_input(company_code)
        ...
        for table_name in self.WORKER_ERROR_TABLE_CANDIDATES:
            columns = self._query_table_columns(table_name)
            if not columns:
                continue
            if "worker_sk" not in columns or "error_code" not in columns:
                logger.info(
                    f"Skipping {self.schema}.{table_name}; worker_sk/error_code columns are required"
                )
                continue

            filters = [
                "w.branch_code = '{branch_code}'",
                "w.company_code = '{company_code}'",
                f"{self._filing_impact_condition('ec')}",
            ]
            if "resolution_status" in columns:
                filters.append("UPPER(COALESCE(e.resolution_status, '')) = 'OPEN'")
            if "write_status" in columns:
                filters.append("UPPER(COALESCE(e.write_status, '')) = 'PENDING'")
            if "year" in columns:
                filters.append("e.year = '{year}'")
            if "quarter" in columns:
                filters.append("e.quarter = '{quarter}'")
            if "organization_unit_sk" in columns:
                filters.extend(
                    [
                        "ou.site_id = '{site_id}'",
                        "ou.branch_code = '{branch_code}'",
                        "ou.company_code = '{company_code}'",
                    ]
                )
            ...
            if "organization_unit_sk" in columns:
                org_join = (
                    f"INNER JOIN {self.schema}.organization_unit ou "
                    "ON e.organization_unit_sk = ou.organization_unit_sk"
                )

            payroll_filter = ""
            if "year" not in columns or "quarter" not in columns:
                payroll_match = ""
                if "payroll_run_sk" in columns:
                    payroll_match = "AND wt.payroll_run_sk = e.payroll_run_sk"
                org_match = ""
                if "organization_unit_sk" in columns:
                    org_match = "AND wt.organization_unit_sk = e.organization_unit_sk"
                payroll_filter = f"""
                AND EXISTS (
                    SELECT 1
                    FROM {self.schema}.worker_tax_qtr_snapshot wt
                    WHERE wt.worker_sk = e.worker_sk
                      {org_match}
                      {payroll_match}
                      AND wt.year = '{year}'
                      AND wt.quarter = '{quarter}'
            ...
            query = f"""
            SELECT DISTINCT e.worker_sk
            FROM {self.schema}.{table_name} e
            INNER JOIN {self.schema}.error_catalog ec
                ON e.error_code = ec.error_code
            INNER JOIN {self.schema}.worker w
                ON e.worker_sk = w.worker_sk
            {org_join}
            WHERE {' AND '.join(formatted_filters)}
            {payroll_filter}
            """
            ...
            try:
                df = self._execute_query(query, timeout_seconds=120)
                rows = df.collect()
                table_worker_sks = {str(row["worker_sk"]) for row in rows}
                if table_worker_sks:
                    logger.info(
                        f"Found {len(table_worker_sks)} filing-impact worker error(s) "
                        f"in {self.schema}.{table_name}"
                    )
                worker_sks.update(table_worker_sks)
            except Exception as e:
                logger.warning(f"Worker error query failed for {self.schema}.{table_name}: {e}")
